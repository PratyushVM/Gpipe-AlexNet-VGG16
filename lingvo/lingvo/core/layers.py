...
class BaseConv2DLayer(quant_utils.QuantizableLayer):
    ...
    def FProp(self, theta, inputs, paddings=None):
        ...
        with tf.name_scope(p.name):
            ...
            if p.conv_last:
                out = self._ComputeConvLast(
                    theta, inputs, paddings, conv_padding)
            else:
        ...
        if p.batch_norm:
            if out_padding is None:
                ...
                out = py_utils.with_dependencies([
                    py_utils.assert_shape_match(batch_time, [-1, -1]),
                    py_utils.assert_shape_match(
                        tf.shape(out), batch_time_any_any)
                ], out)
...
class ProjectionLayer(quant_utils.QuantizableLayer):
    ...
    def Params(cls):
        ...
        p.qdomain.Define(
            'weight', None, 'Quantization domain for the weights.')
        ...
        with tf.name_scope(p.name):
            ...
            if paddings is None:
                shape = tf.convert_to_tensor(
                    py_utils.GetShape(inputs)[:-1], tf.int32)
                paddings = tf.zeros(
                    tf.concat([shape, [1]], axis=0), dtype=inputs.dtype)
        ...
        if not p.use_blocked_matmul:
            ...
            if p.use_einsum:
                if self.apply_compression:
                    ...
                    out = py_utils.ProjectLastDim(
                        inputs, w, p.input_dim, p.output_dim)
            else:
                out = py_utils.Matmul(
    ...
    def FPropMeta(cls, p, inputs, paddings=None):
        ...
        with tf.Graph().as_default():  # throw-away graph.
            ...
            for fc in instance.fc:
                ...
                proj_shape = tshape.Shape(
                    inputs[:-1] + [proj_params.input_dim])
...
class StackingOverTime(base_layer.BaseLayer):
    ...
    def _ApplyStack(self, inputs, pad_value=0.0):
        ...
        if p.left_context == 0 and p.right_context == 0:
            ...
            if p.pad_with_left_frame:
                left_pad = tf.repeat(
                    inputs[:, :1, :], repeats=p.left_context, axis=1)
    ...
    def FProp(self, inputs, paddings=None):
        ...
        with tf.name_scope(p.name):
            ...
            if p.padding_reduce_option == 'reduce_min':
                out_paddings = tf.reduce_min(
                    out_paddings, axis=2, keepdims=True)
            else:
                out_paddings = tf.reduce_max(
                    out_paddings, axis=2, keepdims=True)
    ...
    def Unstack(self, stacked):
        ...
        window_and_frame_indices = tf.stack(
            [window_index, frame_index], axis=1)
    ...
    def FProp(
        ...
        if paddings is not None:
            inputs = py_utils.with_dependencies([
                py_utils.assert_shape_match(tf.shape(paddings), [-1, -1]),
                py_utils.assert_shape_match(
                    tf.shape(inputs)[:2], tf.shape(paddings))
            ], inputs)
        with tf.name_scope(p.name):
            if paddings is not None:
            ...
            if paddings is not None and p.pooling_type == 'AVG':
                ...
                out *= tf.math.reciprocal_no_nan(
                    non_padding_ratio)[..., tf.newaxis]

            out = self.QTensor('output', out)
            if out_padding is not None:
                out *= tf.expand_dims(tf.expand_dims(1.0 -
                                      out_padding, -1), -1)
                return out, out_padding
            return out

    @classmethod  # added fix to include meta flops for pooling
    def FPropMeta(cls, p, inputs, *args):
        py_utils.CheckShapes((inputs,))
        temp = PoolingLayer(p)
        shape = temp.OutShape(inputs)
...
class BlurPoolLayer(base_layer.BaseLayer):
    ...
    def Params(cls):
        p = super().Params()
        p.Define('blur_filter', 'B5',
                 'One of [R2, T3, B5]; the fixed blur filter.')
        p.Define('subsample_type', '1D',
                 'Choose between [1D, 2D] subsampling.')
...
class SimpleEmbeddingLayer(quant_utils.QuantizableLayer):
    ...
    def _FpropImpl(self, embs, ids_vec):
        ...
        def EmbBprop(xs, ys, dys):
            ...
            dembs = inplace_ops.empty(
                weight_shape, py_utils.FPropDtype(p), init=True)
            ...
            def EmbBpropLoop(i, state):
                ...
                # row = state.drets[i]
                row = tf.reshape(tf.gather(state.drets, i),
                                 [1] + emb_shape_suf)
                # state.dembs[row_id] = row
                state.dembs = inplace_ops.alias_inplace_add(
                    state.dembs, [row_id], row)
                return state
        ...
        def EmbFprop(xs):
            ...
            """
            num = tf.shape(xs.ids_vec)[0]
            rets = inplace_ops.empty(
                [num] + emb_shape_suf, py_utils.FPropDtype(p))

            def EmbFpropLoop(i, state):
                # row_id = state.ids_vec[i]
                row_id = tf.gather(state.ids_vec, i)
                # row = state.embs[row_id]
                row = tf.reshape(tf.gather(state.embs, row_id), [
                                 1] + emb_shape_suf)
                # state.rets[i] = row
                state.rets = inplace_ops.alias_inplace_update(
                    state.rets, [i], row)
                return state
            ...
            if len(weight_shape) > 2:
                rets = tf.reshape(
                    rets, [num, symbolic.ToStatic(p.embedding_dim)])
            return rets
    ...
    def _GetWeightShape(self):
        ...
        if py_utils.tpu_compat() and self._fprop_mode != 'matmul':
            if p.use_3d_weight_tensor:
                ...
                emb_shape_suf = [symbolic.ToStatic(
                    p.embedding_dim) // 128, 128]
            else:
...
class SimpleFullSoftmax(SoftmaxLayer):
    ...
    def DenseWeights(self, theta):
        ...
        if p.use_bias:
            biases = [self.QWeight(theta['bias_%d' % i])
                      for i in range(p.num_shards)]
    ...
    def _XentLossByChunk(self, theta, activation, class_ids):
        ...
        acc, _ = recurrent.Recurrent(
            ...
            inputs=py_utils.NestedMap(
                activation=tf.reshape(padded_activation, [
                                      dim0, dim1, input_dim]),
    ...
    @classmethod  # added fix to include meta flops for simplefullsoftmax
    def FPropMeta(cls, p, inputs, *args):
        dim1, dim2 = inputs[:2]
...
class EinsumSoftmax(base_layer.BaseLayer):
    ...
    def Logits(self, theta, inputs):
        ...
        if inputs.shape is not None and inputs.shape.rank < 26:
            ...
            r = inputs.shape.rank
            logits = tf.einsum(
                '{0}y,yz->{0}z'.format(s[:r - 1]), inputs, theta.w)
        else:
...
class SharedSoftmaxLayer(base_layer.BaseLayer):
    ...
    def __init__(self, params):
        ...
        if p.vocab_size != softmax_params.num_classes:
            raise ValueError(
                'SharedSoftmaxLayer vocab_size must equal num_classes.')
...
class SingleShardFullSoftmax(SoftmaxLayer):
    ...
    def XentLossByChunk(self, theta, activation, class_ids, class_probabilities):
        ...
        amax_state0 = tf.zeros(tf.shape(activation)[
                               1:-1], dtype=argmax_out_dtype)
    ...
    def FProp(self,
        ...
        if class_probabilities is not None:
            class_probabilities = py_utils.HasShape(
                class_probabilities, probs_shape)
...
class SingleShardSharedEmbeddingSoftmax(SingleShardFullSoftmax):
    ...
    def Params(cls):
        ...
        p.Define(
            'scale_sqrt_depth', False, 'If set True, activations are scaled'
            ' with sqrt(embedding_dim) in EmbLookup.')
        p.Define('emb_with_matmul', False,
                 'use one-hot vector to perform matmul.')
        return p
...
class ConvSoftmax(quant_utils.QuantizableLayer):
    ...
    def Logits(self, theta, inputs):
        ...
        with tf.name_scope(p.name):
            ...
            logits = tf.nn.bias_add(tf.nn.conv1d(
                x, theta.w, 1, 'VALID'), theta.b)
            if inputs.shape.ndims == 2:
                return logits[:, 0, :]
            else:
                return logits
            ...
            if p.noise_shape_broadcast_dims:
                ...
                for dim in p.noise_shape_broadcast_dims:
                    if dim >= len(noise_shape):
                        raise ValueError(
                            'Invalid broadcasted dim {}'.format(dim))
                    noise_shape[dim] = 1
            else:
...
class LayerNorm(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define('use_fused_layernorm', False,
                 'Whether to use fused layernorm.')
        p.Define(
    ...
    def FProp(self, theta, inputs):
        ...
        with tf.name_scope(p.name):
            ...
            if p.use_fused_layernorm:
                if not p.center:
                    raise ValueError(
                        'use_fused_layernorm does not support center=false.')
            ...
            def Normalize(xs):
                ...
                variance = tf.reduce_mean(
                    tf.square(x_cast), axis=[1], keepdims=True)
...
class ReshapedLayerNorm(LayerNorm):
    ...
    def FProp(self, theta, inputs):
        ...
        with tf.name_scope(p.name):
            ...
            if do_reshape:
                inputs = gshard_utils.ReshapeDim(
                    inputs, 2, p.device_mesh.shape[1])
            inputs = self._CastToFPropDtype(inputs)
...
class CategoricalLayerNorm(LayerNorm):
    ...
    def _CreateLayerVariables(self):
        ...
        super(LayerNorm, self)._CreateLayerVariables(
    ...
    def _GetScaleAndBias(self, theta):
        ...
        with tf.control_dependencies(
            ...
            biases = [theta[self._BiasVarName(i)]
                      for i in range(p.num_classes)]
            cur_bias = tf.gather(biases, theta.class_index)
            scales = [theta[self._ScaleVarName(i)]
                      for i in range(p.num_classes)]
    ...
    def FProp(self, theta, target_paddings, target_labels, target_ids):
        ...
        for offset, weight in zip(p.offsets, p.weights):
            ...
            output_distributions.append(
                class_probabilities[:,
                                    offset_in_padded:offset_in_padded + seq_len, :]
...
class HighwaySkipLayer(base_layer.BaseLayer):
    ...
    def FProp(self, theta, x, transformed_x, paddings=None):
        ...
        if p.couple_carry_transform_gates:
            ...
            transform = self.transform_gate.FProp(
                theta.transform_gate, x, paddings)
...
class GradNormTracker(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define('grad_norm_lower_cap', 1e-2,
                 'The minimal gradient norm value.')
        p.Define(
            'clip_threshold', 4.0,
    ...
    def FProp(self, theta, grad_norm, has_nan=None):
        ...
        with tf.name_scope(p.name):
            ...
            log_grad_norm_cap = tf.cast(
                mean + std * p.clip_threshold, tf.float32)
            log_grad_norm_cap_min = tf.math.log(p.grad_norm_clip_cap_min + 1.0)
            log_grad_norm_cap = tf.maximum(
                log_grad_norm_cap, log_grad_norm_cap_min)
            ...
            update_moving_avg = tf.group(
                UpdateExpMovingAvg(self.vars.log_mean,
                                   log_grad_norm_capped, has_nan),
                UpdateExpMovingAvg(self.vars.log_mean_squared,
                                   log_grad_norm_capped * log_grad_norm_capped,
                                   has_nan),
                UpdateExpMovingAvg(self.vars.total_weight,
                                   tf.constant(1.0), has_nan),
...
class WeightedSumLayer(base_layer.BaseLayer):
    ...
    def FProp(self, theta, inputs):
        ...
        w = self.weighted_merger_dropout.FProp(
            theta.weighted_merger_dropout, w)
        ...
        if p.weighted_merger_softmax:
            ...
            w = tf.nn.softmax(w, axis=0) * \
                (1.0 - residual_weights) + p.minimal_prob
...
class GatedAverageLayer(base_layer.BaseLayer):
    ...
    def FProp(self, theta, inputs):
        ...
        p = self.params
        assert len(
            inputs) == p.num_inputs, 'Number of inputs should match params.'

        for i, inp in enumerate(inputs):
            inputs[i] = py_utils.with_dependencies([
                py_utils.assert_shape_match(
                    [tf.shape(inp)[-1]], [p.num_nodes]),
                py_utils.assert_shape_match(
                    tf.shape(inp), tf.shape(inputs[0])),
            ], inp)

        input_shape = tf.shape(inputs[0])

        reshaped_inputs = [tf.reshape(inp, [-1, p.num_nodes])
                           for inp in inputs]
...
class ResidualAdapterLayer(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define('bottleneck_dim', 0,
                 'Dimension of the feedforward inner layer.')
...
class Conv2DLayerNoPadding(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define(
            'dilations', (1,
                          1), ' An optional list of ints. Defaults to [1, 1]. '
    ...
    def FPropMeta(cls, p, inputs):
        ...
        # mul/add counts as 2 flop.
        flops = b * oh * ow * fh * fw * ic * oc * 2
...
class FetchLayer(base_layer.BaseLayer):
    ...
    def FProp(self, theta, *args):
        ...
        for i, v in enumerate(args):
            ...
            self._activations[i] = py_utils.CallDefun(
                FetchFwd, v, bak=FetchBak)
...
class GluLayer(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define('dropout_tpl', DropoutLayer.Params(),
                 'Dropout applied to output.')
        p.Define('apply_residual', True,
                 'Whether or not to add inputs to outputs.')
        return p
    ...
    def FProp(self, theta, inputs, paddings):
        ...
        values = self.value_layer.FProp(theta.value_layer, inputs_normalized,
                                        paddings)
        gates = self.gate_layer.FProp(
            theta.gate_layer, inputs_normalized, paddings)
...
class MultitaskAdapterBaseLayer(base_layer.BaseLayer):
    ...
    def Params(cls):
        ...
        p.Define('layer_norm_tpl', LayerNorm.Params(),
...
class MultitaskAdapterLayer(MultitaskAdapterBaseLayer):
    ...
    def __init__(self, params):
        ...
        down_proj_b_params.Set(
            embedding_dim=p.bottleneck_dim, name='down_proj_b')
    ...
    def FProp(self, theta, inputs, tasks):
        ...
        if p.data_format == 'TBC':
            down_projected = tf.einsum(
                'tbh,bhk->tbk', norm_inputs, down_weights)
        else:
            down_projected = tf.einsum(
                'bth,bhk->btk', norm_inputs, down_weights)
        down_projected += down_biases
        down_projected = tf.nn.relu(down_projected)
        if p.data_format == 'TBC':
            up_projected = tf.einsum(
                'tbk,bkh->tbh', down_projected, up_weights)
        else:
            up_projected = tf.einsum(
                'btk,bkh->bth', down_projected, up_weights)
...
class MultitaskAdapterEinsumLayer(MultitaskAdapterBaseLayer):
    ...
    def FProp(self, theta, inputs, tasks):
        ...
        down_projected = tf.einsum(
            'bti,bin->btn', norm_inputs, down_w) + down_b
        # ReLU.
        down_projected = tf.nn.relu(down_projected)
        # [batch, time, input_dim].
...
class CCTGatingNetwork(quant_utils.QuantizableLayer):
    ...
    def FProp(self, theta, inputs, paddings=None):
        ...
        if self.do_eval:
            ...
            p_c = tf.where(
                tf.greater_equal(p_c, tf.constant(
                    0.0, dtype=py_utils.FPropDtype(p))),
                ones, zeros)
        else:
    ...
    def FPropMeta(cls, p, inputs, paddings=None):
        ...
        out_shape = tshape.Shape(
            inputs[:-1] + [symbolic.ToStatic(p.num_outputs)])
        return py_utils.NestedMap(flops=flops, out_shapes=(out_shape,))
