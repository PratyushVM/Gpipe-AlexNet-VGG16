...
class _Recurrent:
    ...
    def __init__(self,
        ...
        def BackwardLoopBody(loop_state):
            ...
            d_state1 = _Add(_Index(loop_state.d_acc_state, t),
                            loop_state.d_state1)
            ...
            if self._unused_acc_state:
                ...
                loop_state.d_state1 = loop_state.d_state1.Transform(
                    tf.reduce_sum)
    ...
    def Compute(self):
        ...
        if self._accumulator_layer:
            ...
            self._accumulator_layer.SetAccumulatorValues(
...
def _ReflectOnCellFn(cell_fn,
    ...
    if Fwd.stateful_ops:
        if check_stateful_ops:
            raise ValueError('cell_fn contains stateful ops: %s' %
                             Fwd.stateful_ops)
        else:
            tf.logging.warning(
                'cell_fn contains stateful ops: %s', Fwd.stateful_ops)
    ...
    if captured_inputs:
        if allowed_tensor_captures:
            allowed_tensor_names = set(
                [x.name for x in allowed_tensor_captures])
...
def _GetCellGrad(cell_fn,
    ...
    if not cell_grad:
        ...
        def CellGrad(theta, state0, inputs, unused_extras, dstate1):
            ...
            # we are running in.
            captured = py_utils.Pack(
                implicit_captures, py_utils.GetExtraArgs())
...
def _AugmentState(state0, accumulator_layer, allow_overwrite=False):
    ...
    if accumulator_layer:
        if 'accumulators' in state0 and not allow_overwrite:
            raise ValueError(
                'accumulators is a private state key used by Recurrent.')
    ...
    if '_step_seed' in state0 and not allow_overwrite:
        raise ValueError(
            '_step_seed is a private state key used by Recurrent.')
...
def Recurrent(theta,
    ...
    """
    symbol_to_tensor_map = symbolic.SymbolToValueMap.Get(
    ...
    if extras is None:
        with tf.name_scope('recurrent_cellfn_extras'):
            ...
            _, actual_extras = cell_fn(
                theta, state0.DeepCopy(), _Index(inputs, 0))
...
def _StackedRecurrent(devices, cell_fns, cell_grads, cell_outs, cell_out_grads,
    ...
    for i in range(num_layers):
        with tf.device(devices[i]):
            thetas[i], init_states[i] = _DependsOn(
                [thetas[i], init_states[i]], prev)
    ...
    def ExpectedOutputOfLayers():
        ...
        for i in range(num_layers):
            ...
            if accumulator_layers[i]:
                accumulator_layers[i].accumulators.Transform(
                    lambda x: x.Disable())
            step_seed = py_utils.GetStepSeed()
    ...
    for i in range(1, num_layers - 1):
        ...
        mid_l = _Middle(
            ...
            theta=thetas[i],
            state0=_AugmentState(
                init_states[i].DeepCopy(), accumulator_layers[i]),
    ...
    out_l = _Output(
        ...
        theta=thetas[-1],
        state0=_AugmentState(
            init_states[-1].DeepCopy(), accumulator_layers[-1]),
    ...
    for (dev, layer) in zip(devices, layers):
        ...
        with tf.device(dev):
            # Don't care of final state yet.
            acc_states, final = layer.Compute()
