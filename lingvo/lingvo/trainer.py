...
tf.flags.DEFINE_integer(
    'decoder_gpus', 0, 'Number of gpus to use per replica.')
            ...
            while True:
                ...
                if self._trial.ShouldStopAndMaybeReport(global_step, eval_metrics):
                    ...
                    if self._max_steps is None:
                        ...
                        tf.logging.info(
                            'Early stopping at step: %d', self._max_steps)
                ...
                self._SummarizeValue(
                    global_step, 'total_samples', total_examples)
                if FLAGS.checkpoint_in_trainer_tpu:
                    self._SummarizeValue(global_step, 'total_num_params',
                                         self._total_num_params)
    ...
    def _Loop(self):
        ...
        with tf.container(
            ...
            if self._eval_path:
                ...
                self._UpdateProcessedCheckpoints(
                    self._eval_dir, self._eval_path)
                ...
                self._RunOnLatestCheckpoints(
                    sess, self._EvalOnce, self._eval_dir)
    ...
    def _EvalOnce(self, sess, path):
        ...
        while samples_per_summary == 0 or (num_samples_metric.total_value <
            ...
            try:
                ...
                if self._summary_op is not None and is_first_loop:
                    ans, summaries = sess.run(
                        [self._task.eval_metrics, self._summary_op])
        ...
        if self._task.input.merged_input_data_summary_op is not None:
            input_stats_summary_str = sess.run(
                self._task.input.merged_input_data_summary_op)
            self._WriteInputDataStatSummaries(
                input_stats_summary_str, global_step)

        if self._should_report_metrics:
            tf.logging.info(
                'Reporting eval measure for step %d.' % global_step)
...
class RunnerManager:
    ...
    def GetParamsForDataset(self, job_name, dataset_name):
        ...
        with cluster_factory.Cluster(cluster.params):
            try:
                cfg = self.model_registry.GetParams(
                    self._model_name, dataset_name)
    ...
    def MaybeConfigRunDistributed(self):
        ...
        for job in cluster_spec_dict:
            if job.startswith('decoder_'):
                assert len(
                    job_specs) == 1, 'Decoder jobs must run on their own'
            ...
            if FLAGS.additional_worker_jobs:
                for additional_job in cluster.worker.additional_worker_names:
                    additional_job_name = additional_job.replace(
                        '/job:', '', 1)
                    worker_hosts.extend(_GetClusterSpecDict()[
                                        additional_job_name])
    ...
    def StartRunners(self, runners):
        ...
        for t in threads:
            ...
            while True:
                t.join(1)
                if not t.is_alive():  # isAlive() used previously, became deprecated in python3.9
                    break
    ...
    def InspectParams(self):
        ...
        for dataset in datasets.GetDatasets(cls):
            ...
            tf.logging.info(
                'Write all params for {} to {}'.format(dataset, outf))
        ...
        if FLAGS.inference_graph_filename:
            ...
            for task_name in task_names:
                ...
                filename_prefix = os.path.join(
                    inference_graph_dir, filename_prefix)
    ...
    def Start(self):
        ...
        self.StartRunners(self.CreateRunners(
            FLAGS.job.split(','), FLAGS.logdir))
