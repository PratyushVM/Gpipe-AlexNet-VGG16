...
class Trainer(base_runner.BaseRunner):
    ...
    def _Loop(self):
        ...
        with tf.container(
            ...
            eval_metrics = None
            while True:
                if(tf.flags.FLAGS.use_profiler == True):
                    if(global_step == tf.flags.FLAGS.profiler_start):
                        tf.profiler.experimental.start('logs')
                    elif(global_step == tf.flags.FLAGS.profiler_stop):
                        tf.profiler.experimental.stop()
                if (self._trial.ShouldStopAndMaybeReport(global_step, eval_metrics) or
                        self._ShouldStop(sess, global_step)):
                    tf.logging.info('Training finished.')
                    if self._early_stop:
                        # controller hangs if it doesn't finish first
                        time.sleep(300)
...
class Decoder(base_runner.BaseRunner):
        ...
        with tf.container(self._container_id), self._cluster, self._GetSession(
            ...
            if self._decode_path:
                ...
                self._UpdateProcessedCheckpoints(
                    self._decoder_dir, self._decode_path)
    ...
    def DecodeCheckpoint(self, sess, checkpoint_path):
        ...
        checkpoint_id = _GetCheckpointIdForDecodeOut(
            ckpt_id_from_file, global_step)
        decode_out_path = self.GetDecodeOutPath(
            self._decoder_dir, checkpoint_id)
